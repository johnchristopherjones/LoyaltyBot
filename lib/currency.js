        ...
        if (broadcaster_bot_initiated || (__self.config.modpowers && moderator_initiated)) {
            ...
            if (data[4] === 'auction') {
                ...
                case 'cancel':
                    ...
                    break;
                case 'help':
                    __self.irc.emit('message', {message:__self.pre_text + ' Help: !' + __self.config.currency.toLowerCase() + ' auction open/close/cancel/draw'});
                    break;
                }
            }
            ...
            if (data[4] === 'raffle') {
                ...
                case 'restore':
                    ...
                    break;
                case 'help':
                    __self.irc.emit('message', {message:__self.pre_text + ' Help: !' + __self.config.currency.toLowerCase() + ' raffle open <ticket cost> <tickets per user>/close/cancel/draw/restore'});
                    break;
                }
            }
            ...
            if (data[4] === 'bet') {
                switch (data[5]) {
                    ...
                    case 'cancel':
                        ...
                        break;
                    case 'help':
                        __self.irc.emit('message', {message:__self.pre_text + ' Help: !' + __self.config.currency.toLowerCase() + ' bet open <option A> <option B> <etc>/close/pool/winner <option>/cancel'});
                        break;
                }
            }
    ...
    function do_work() {
        ...
        __self.query_rank(__self.rank_flood, function (ranks, total) {
            ...
            for(var i = 0; i < ranks.length; i++) {
                if (i !== ranks.length - 1) {
                    multi_response += ranks[i]['user'] + ' ('+ranks[i]['@row_number:=@row_number+1']+'), ';
                } else {
                    multi_response += ranks[i]['user'] + ' ('+ranks[i]['@row_number:=@row_number+1']+')';
                }
            }
            __self.irc.emit('message', {message: __self.pre_text + ' Rank (of ' + total + '): ' + multi_response, timer: 1});
        });
...
Currency.prototype.get_top = function (data) {
    ...
    function do_work() {
        ...
        __self.query_top(__self.top_flood, function (rows) {
            ...
            __self.irc.emit('message', {message:__self.pre_text + ' Top ' + rows.length + ': ' + response});
        });
